...
type
  ...
  {$IFDEF QDAC_UNICODE}
  IQPtr<T> = interface(IInterface)
    function Get:T;
  end;

  TQPtr<T> = class(TInterfacedObject,IQPtr<T>)
  private
    FObject:T;
  public
    constructor Create(AObject:T);overload;
    destructor Destroy;override;
    class function Bind(AObject:T):IQPtr<T>;
    function Get:T;
  end;
  {$ENDIF}
...
begin
p:=PQCharW(S);
SetLength(Result, System.Length(S) shl 1);
pe:=p;
Inc(pe,System.Length(S));
pd:=PQCharW(Result);
pds:=pd;
pd^:=AQuoter;
Inc(pd);
...
class function TQPtr<T>.Bind(AObject: T): IQPtr<T>;
begin
Result:=TQPtr<T>.Create(AObject) as IQPtr<T>;
end;

constructor TQPtr<T>.Create(AObject: T);
begin
inherited Create;
FObject:=AObject;
end;

destructor TQPtr<T>.Destroy;
begin
FreeAndNil(FObject);
  inherited;
end;

function TQPtr<T>.Get: T;
begin
Result:=FObject;
end;
{$ENDIF QDAC_UNICODE}
