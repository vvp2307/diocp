...
type
  ...
  protected
    ...
    function GetIsEmpty: Boolean;inline;
    procedure RotateSetParents(old, new:TQRBNode;color:Integer);inline;
    procedure InsertNode(node:TQRBNode);inline;
    procedure EraseColor(AParent:TQRBNode);inline;
    procedure ChangeChild(old,new,parent:TQRBNode);inline;
  ...
  public
    ...
    /// <param name="new">新结点</param>
    /// <remarks>替换要自己保证内容和牺牲品一致，否则可能造成树错乱，如果不能保证,
    /// 尝试删除+添加来完成替换
    /// </remarks>
    procedure Replace(victim,new:TQRBNode);
    /// 判断树是否为空树
...
procedure TQRBTree.ChangeChild(old, new, parent: TQRBNode);
begin
if parent<>nil then
  begin
...
procedure TQRBTree.Replace(victim, new: TQRBNode);
var
  parent:TQRBNode;
begin
parent := victim.Parent;
ChangeChild(victim,new,parent);
if Assigned(victim.Left) then
  victim.Left.SetParent(new)
else
  victim.Right.SetParent(new);
new.Assign(victim);
end;
...
procedure TQRBTree.RotateSetParents(old, new: TQRBNode; color: Integer);
var
  AParent:TQRBNode;
begin
AParent:=old.Parent;
new.FParent_Color:=old.FParent_Color;
old.SetParentColor(new,color);
ChangeChild(old,new,AParent);
end;
